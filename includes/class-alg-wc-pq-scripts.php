	...
	function enqueue_scripts() {
		...
		if (
			( $do_load_all_variations = (
				'yes' === get_option( 'alg_wc_pq_variation_do_load_all', 'no' ) )
			) ||
			(
			...
			$quantities_options = array(
				'reset_to_min'           => (
					'reset_to_min' === get_option( 'alg_wc_pq_variation_change', 'do_nothing' )
				),
				'reset_to_max'           => (
					'reset_to_max' === get_option( 'alg_wc_pq_variation_change', 'do_nothing' )
				),
				'reset_to_default'       => (
					'reset_to_default' === get_option( 'alg_wc_pq_variation_change', 'do_nothing' )
				),
				'reset_to_lowest_fixed'  => (
					'reset_to_lowest_fixed' === get_option( 'alg_wc_pq_variation_change', 'do_nothing' )
				),
				'do_load_all_variations' => $do_load_all_variations,
				'max_value_fallback'     => $max_value_fallback = get_option(
					'alg_wc_pq_qty_dropdown_max_value_fallback',
					100
				),
				...
				foreach (
					...
					if ( $_product = wc_get_product( $product_id ) ) {
						foreach ( $_product->get_available_variations() as $variation ) {
							...
							$step      = alg_wc_pq()->core->get_product_qty_step(
								$product_id,
								1,
								$variation['variation_id']
							);
							$label     = alg_wc_pq()->core->get_dropdown_option_label(
								$variation['variation_id']
							);
							$default   = alg_wc_pq()->core->get_product_qty_default(
								$variation['variation_id'], 1
							);
							$exact_qty = alg_wc_pq()->core->get_product_exact_qty(
								$product_id,
								'allowed',
								'', $variation_id
							);
							...
							$product_quantities[ $variation['variation_id'] ] = array(
								...
								'default'       => $default,
								'exact_qty'     => $exact_qty,
								'vprice_by_qty' => $this->alg_wc_pq_prices_by_variation(
									$variation_id,
									$min_qty,
									$max_qty,
									$step,
			...
			alg_wc_pq_enqueue_script(
				'alg-wc-pq-variable',
				trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-variable.js',
				array( 'jquery' ),
				alg_wc_pq()->version,
				true
			);
		...
		if ( 'yes' === get_option( 'alg_wc_pq_qty_price_by_qty_enabled', 'no' ) ) {
			if (
				...
				if ( $_product->is_purchasable() ) {
					alg_wc_pq_enqueue_script(
						'alg-wc-pq-price-by-qty',
						trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-price-by-qty.js',
						array( 'jquery' ),
						alg_wc_pq()->version,
						true
					);
				...
				if (
					...
					if (
						'yes' === get_option( 'alg_wc_pq_qty_price_by_qty_qty_archive_enabled', 'no' ) &&
						'yes' === get_option( 'alg_wc_pq_add_quantity_archive_enabled', 'no' )
					) {
						alg_wc_pq_enqueue_script(
							'alg-wc-pq-price-by-qty',
							trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-price-by-qty.js',
		...
		if (
			'yes' === get_option( 'alg_wc_pq_qty_price_by_qty_enabled', 'no' ) &&
			'yes' === get_option( 'alg_wc_pq_qty_price_by_qty_enabled_variable', 'no' ) &&
		...
		if ( 'yes' === get_option( 'alg_wc_pq_step_section_enabled', 'no' ) ) {
			$force_check_step_periodically = (
				'yes' === get_option( 'alg_wc_pq_force_js_check_step_periodically', 'no' )
			);
			$force_check_step_on_change    = (
				'yes' === get_option( 'alg_wc_pq_force_js_check_step', 'no' )
			);
			if (
				$force_check_step_periodically ||
				$force_check_step_on_change
				...
				alg_wc_pq_enqueue_script(
					'alg-wc-pq-force-step-check',
					trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-force-step-check.js',
					array( 'jquery' ),
					alg_wc_pq()->version,
					true
				);
				wp_localize_script(
					...
					array(
						...
						'force_check_step_periodically_ms' => get_option(
							'alg_wc_pq_force_js_check_period_ms',
							1000
						),
					)
				);
		...
		if (
			...
			$force_check_min_max_periodically = (
				'yes' === get_option( 'alg_wc_pq_force_js_check_min_max_periodically', 'no' )
			);
			$force_check_min_max_on_change    = (
				'yes' === get_option( 'alg_wc_pq_force_js_check_min_max', 'no' )
			);
			if (
				$force_check_min_max_periodically ||
				$force_check_min_max_on_change
				...
				alg_wc_pq_enqueue_script(
					'alg-wc-pq-force-min-max-check',
					trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-force-min-max-check.js',
					array( 'jquery' ),
					alg_wc_pq()->version,
					true
				);
		...
		// Qty rounding
		if ( 'no' != (
			$round_with_js_func = get_option( 'alg_wc_pq_round_with_js', 'no' ) )
		) {
			alg_wc_pq_enqueue_script(
				'alg-wc-pq-force-rounding',
				trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-force-rounding.js',
				array( 'jquery' ),
				alg_wc_pq()->version,
				true
			);
		...
		if (
			'yes' === get_option( 'alg_wc_pq_qty_dropdown', 'no' ) &&
			'yes' == get_option( 'alg_wc_pq_qty_dropdown_search_filter', 'no' )
		) {
			wp_register_style(
				'select2-algwc',
				trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/css/alg-wc-select2.css',
				false,
				'1.0',
				'all'
			);
			wp_register_script(
				'select2-algwc',
				trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-select2.js',
				array( 'jquery' ),
				'1.0',
				true
			);
		...
		$alg_wc_pq_step_per_item_quantity_allow_all_remaining = get_option(
			'alg_wc_pq_step_per_item_quantity_allow_all_remaining',
			'no'
		);
		...
		if (
			...
			$message_template_overflow = get_option(
				'alg_wc_pq_max_per_item_message',
				__(
			...
			$message_template_underflow = get_option(
				'alg_wc_pq_min_per_item_message',
				__(
			...
			alg_wc_pq_enqueue_script(
				'alg-wc-pq-invalid-html5-browser-msg',
				trailingslashit( alg_wc_pq()->plugin_url() ) . 'includes/js/alg-wc-pq-invalid-html5-browser-msg.js',
				array( 'jquery' ),
				alg_wc_pq()->version,
				true
			);
