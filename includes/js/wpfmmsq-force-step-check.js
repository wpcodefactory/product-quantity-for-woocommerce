/**
 * wpfmmsq-force-step-check.js
 *
 * @version 5.3.1
 * @since   1.6.2
 * @todo    [dev] (important) (maybe) fire some event (e.g. `change`) on correction (e.g. for `wpfmmsq-price-by-qty.js`)
 * @todo    [feature] (maybe) add separate option for the cart (same in `wpfmmsq-force-min-max-check.js`)
 */

function wpfmmsq_check_qty_step_for_input( qty_input ) {
	if ( qty_input.length ) {
...
function wpfmmsq_check_qty_step_all() {
	jQuery( 'input[id^=quantity_]' ).each( function () {
		wpfmmsq_check_qty_step_for_input( jQuery( this ) )
	} );
}

if ( wpfmmsq_force_step_check_options.force_check_step_periodically ) {
	setInterval( wpfmmsq_check_qty_step_all, wpfmmsq_force_step_check_options.force_check_step_periodically_ms );
}

if ( wpfmmsq_force_step_check_options.force_check_step_on_change ) {
