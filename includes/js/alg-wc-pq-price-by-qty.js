...
jQuery( document ).ready( function () {
	if (
		'instead' != alg_wc_pq_update_price_by_qty_object.position &&
		parseInt( alg_wc_pq_update_price_by_qty_object.product_id ) != 0
	) {
	...
	jQuery( '[name="quantity"]' )
		...
		.on( 'change', alg_wc_pq_update_price_by_qty );
