...
if ( ! class_exists( 'WPFMMSQ_Shortcodes' ) ) :

	class WPFMMSQ_Shortcodes {
		 ...
		 * @since         1.6.0
		 *
		 * @todo    (dev) `[wpfmmsq_min_exact_allowed_product_qty]`
		 * @todo    (dev) `[wpfmmsq_min_exact_disallowed_product_qty]`
		 */
		function __construct() {
			add_shortcode( 'wpfmmsq_min_product_qty', array( $this, 'min_product_qty' ) );
			add_shortcode( 'wpfmmsq_max_product_qty', array( $this, 'max_product_qty' ) );
			add_shortcode( 'wpfmmsq_product_qty_step', array( $this, 'product_qty_step' ) );
			add_shortcode( 'wpfmmsq_product_qty_price_unit', array( $this, 'product_qty_price_unit' ) );
			add_shortcode( 'wpfmmsq_translate', array( $this, 'language_shortcode' ) );
		}
		...
		function get_min_or_max_or_step_value( $product, $min_or_max_or_step ) {
			switch ( $min_or_max_or_step ) {
				case 'min':
					return wpfmmsq()->core->set_quantity_input_min( 1, $product );
				case 'max':
					return wpfmmsq()->core->set_quantity_input_max( 0, $product );
				case 'step':
					return wpfmmsq()->core->set_quantity_input_step( 1, $product, true );
				case 'price_unit':
					return wpfmmsq()->core->set_quantity_input_price_unit( '', $product );
			}
		}
		...
		function get_min_or_max_or_step_value_main_var( $product, $min_or_max_or_step ) {
			switch ( $min_or_max_or_step ) {
				case 'min':
					return wpfmmsq()->core->set_quantity_input_min( 0, $product );
				case 'max':
					return wpfmmsq()->core->set_quantity_input_max( 0, $product );
				case 'step':
					return wpfmmsq()->core->set_quantity_input_step( 0, $product );
				case 'price_unit':
					return wpfmmsq()->core->set_quantity_input_price_unit( '', $product );
			}
		}
		...
		function get_min_or_max_or_step_variable( $product, $min_or_max_or_step ) {
			...
			if ( ! empty( $values ) ) {
				...
				$min_value !== $max_value ?
					sprintf(
						'%s &ndash; %s',
						number_format( $min_value, 0, '.', '' ),
						number_format( $max_value, 0, '.', '' )
					) :
					$min_value
				);
			}
		...
		function output_shortcode( $value, $atts ) {
			...
			if ( isset( $atts['thousand_sep'] ) && 'yes' === $atts['thousand_sep'] ) {
				if ( ! empty( $value ) && is_numeric( $value ) ) {
					$value = number_format( $value );
				}
			}

			if ( isset( $atts['price_unit'] ) && 'yes' === $atts['price_unit'] ) {
				$unit = wpfmmsq()->core->wpfmmsq_get_product_price_unit( $product, $value, true );
				if ( ! empty( $unit ) ) {
					$value = $value . ' ' . $unit;
				} else {
					return '';
			...
			if ( isset( $atts['unit'] ) && 'yes' === $atts['unit'] ) {
				$unit = wpfmmsq()->core->wpfmmsq_get_product_price_unit( $product, $value, false );
				if ( ! empty( $unit ) ) {
					$value = $value . ' ' . $unit;
				} else {
					return '';
		...
		function language_shortcode( $atts, $content = '' ) {
			...
			if ( ! empty( $atts['lang'] ) && 2 === strlen( trim( $atts['lang'] ) ) ) {
