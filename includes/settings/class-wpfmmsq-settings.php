...
class WPFMMSQ_Settings extends WC_Settings_Page {
		...
		<div class="alg_wc_pq_right_ad">
			<div class="alg_wc_pq-sidebar__section">
				<div class="alg_wc_pq_name_heading">
					<img class="alg_wc_pq_resize" src="<?php echo esc_url( plugins_url( '../../assets/images/icon-128x128.png', __FILE__ ) ); ?>">
					<p class="alg_wc_pq_text">Enjoying the plugin? Unleash its
						full potential with the premium version, it allows you
						to: </p>
				</div>
				<ul>
					<li>
						<strong>Set Min/Max/Step quantity values per category or
							product, define values the way you want!</strong>
					</li>
					<li>
				...
				<p style="text-align:center">
					<a id="alg_wc_pq-premium-button" class="alg_wc_pq-button-upsell" href="https://wpfactory.com/item/product-quantity-for-woocommerce" target="_blank">Get
						Min Max Step Quantity Limits Manager for WooCommerce Pro</a>
				</p>
				<br>
			</div>
		</div>
	...
	function maybe_reset_settings() {
		...
		if ( count( $this->get_settings() ) > 0 ) {
			foreach ( $this->get_settings() as $value ) {
				if ( isset( $value['id'] ) && 'alg_wc_pq_disable_urls' == $value['id'] ) {
					$ids  = [];
					$urls = ( isset( $_POST['alg_wc_pq_disable_urls'] ) ? sanitize_textarea_field( wp_unslash( $_POST['alg_wc_pq_disable_urls'] ) ) : '' );
				...
				if ( isset( $value['id'] ) && 'alg_wc_pq_disable_by_category' == $value['id'] ) {
					$c_p_ids      = [];
					$category_ids = ( isset( $_POST['alg_wc_pq_disable_by_category'] ) ? array_map( 'absint', (array) wp_unslash( $_POST['alg_wc_pq_disable_by_category'] ) ) : array() );
