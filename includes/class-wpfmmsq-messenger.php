...
if ( ! class_exists( 'WPFMMSQ_Messenger' ) ) :

	class WPFMMSQ_Messenger {
		...
		function print_message( $message_type, $_is_cart, $required_quantity, $total_quantity, $_product_id = 0 ) {
			if ( $_is_cart ) {
				if ( 'no' === get_option( 'wpfmmsq_cart_notice_enabled', 'no' ) ) {
					return;
				}
			}
			...
			switch ( $message_type ) {
				case 'step_cart_total_quantity':
					...
					$message_template = get_option( 'wpfmmsq_step_cart_total_message',
				...
				case 'max_cart_total_quantity':
					...
					$message_template = get_option( 'wpfmmsq_max_cart_total_message',
				...
				case 'min_cart_total_quantity':
					...
					$message_template = get_option( 'wpfmmsq_min_cart_total_message',
				...
				case 'max_per_item_quantity':
					...
					$message_template = get_option( 'wpfmmsq_max_per_item_message',
				...
				case 'min_per_item_quantity':
					...
					$message_template = get_option( 'wpfmmsq_min_per_item_message',
				...
				case 'step_quantity':
					...
					$message_template  = get_option( 'wpfmmsq_step_message',
				...
				case 'exact_qty_allowed':
					...
					$message_template = get_option( 'wpfmmsq_exact_qty_allowed_message',
				...
				case 'exact_qty_disallowed':
					...
					$message_template = get_option( 'wpfmmsq_exact_qty_disallowed_message',
			...
			if ( function_exists( 'pll__' ) ) {
				$message_template = pll__( $message_template );
			} else {
				$message_template = apply_filters( 'widget_title', $message_template );
			}
